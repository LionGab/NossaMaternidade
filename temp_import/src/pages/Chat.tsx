...
export const Chat: React.FC<ChatProps> = ({ onBack, initialMessage }) => {
  ...
  // Load User Profile for Context-Aware Chips
  useEffect(() => {
  ...
  const getChips = () => {
    const defaults = ["Estou sobrecarregada", "Medo de não ser boa mãe", "Briguei com meu parceiro"];
    if (!userProfile) return defaults;
    
    const dynamicChips = [];
    if (userProfile.currentFeeling) dynamicChips.push(`Por que estou ${userProfile.currentFeeling.toLowerCase()}?`);
    if (userProfile.biggestChallenge) dynamicChips.push(`Dicas para ${userProfile.biggestChallenge.toLowerCase()}`);
    
    // Combine and dedup defaults
    return [...dynamicChips, ...defaults].slice(0, 4);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F4F8] dark:bg-nath-dark-bg transition-colors duration-300 pb-[85px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-8 flex flex-col">
        ...
        {messages.length === 0 ? (
           <div className="flex-1 flex flex-col items-center justify-center opacity-100 animate-in fade-in duration-500 -mt-12">
              <div className="w-36 h-36 rounded-full border-4 border-white dark:border-nath-dark-card shadow-xl overflow-hidden mb-4 relative">
                 <img src="https://i.imgur.com/RRIaE7t.jpg" alt="MãesValente" className="w-full h-full object-cover" />
              </div>
              
              <div className="text-center px-6">
                <h2 className="text-3xl font-bold text-nath-dark dark:text-white mb-1">MãesValente</h2>
                <p className="text-nath-blue dark:text-nath-dark-hero font-medium text-sm mb-6 tracking-wide uppercase flex items-center justify-center gap-2">
                  <Sparkles size={14} /> Suas conversas
                </p>
                
                <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed max-w-[260px] mx-auto mb-8">
                  {userProfile?.name ? `Oi, ${userProfile.name.split(' ')[0]}.` : 'Oi, mãe.'} <br/>
                  Como você está se sentindo agora? Posso te ajudar com {userProfile?.biggestChallenge?.toLowerCase() || 'o dia a dia'}?
                </p>
              </div>
           </div>
        ) : (
      ...
      <div className="bg-white dark:bg-nath-dark-tab p-3 border-t border-gray-100 dark:border-nath-dark-border transition-colors">
        {!loading && messages.length < 2 && (
           <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar pb-1 px-1 justify-start md:justify-center">
             {getChips().map((chip, i) => (
               <button 
                key={i} 
                onClick={() => { triggerHaptic(); setInput(chip); handleSend(chip); }}
                className="whitespace-nowrap flex-shrink-0 bg-nath-light-blue dark:bg-nath-dark-card text-nath-blue dark:text-nath-blue text-xs px-4 py-2.5 rounded-full border border-nath-blue/20 dark:border-nath-dark-border hover:bg-nath-blue hover:text-white dark:hover:bg-nath-dark-hero dark:hover:text-white transition-colors active:scale-95 font-medium"
               >
                 {chip}
               </button>
             ))}
