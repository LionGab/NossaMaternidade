...
import { MCPRequest, MCPResponse, createMCPResponse } from '../../src/mcp/types';
...
describe('ToolExecutor', () => {
  ...
  describe('Parallel Execution', () => {
    it('deve executar múltiplas ferramentas em paralelo', async () => {
      ...
      const mockExecutor = async (request: MCPRequest): Promise<MCPResponse> => {
        ...
        return createMCPResponse(request.id, { result: value });
      };

      const result = await executor.executeParallel(calls, mockExecutor);
    ...
    it('deve ser mais rápido que execução sequencial', async () => {
      ...
      await executor.executeParallel(calls, mockExecutor);
      const parallelTime = Date.now() - startParallel;

      const startSequential = Date.now();
      await executor.executeSequential(calls, mockExecutor);
  ...
  describe('Retry Logic', () => {
    it('deve fazer retry em caso de erro recuperável', async () => {
      const calls: ToolCall[] = [
        { server: 'test', method: 'test.flaky', params: {} },
      ];
    ...
    it('deve respeitar maxAttempts', async () => {
      const calls: ToolCall[] = [
        { server: 'test', method: 'test.alwaysFails', params: {} },
      ];
    ...
    it('não deve fazer retry em erros não recuperáveis', async () => {
      const calls: ToolCall[] = [
        { server: 'test', method: 'test.invalidParams', params: {} },
      ];
  ...
  describe('Result Aggregation', () => {
    it('deve agregar resultados corretamente', async () => {
      ...
      const mockExecutor = async (request: MCPRequest): Promise<MCPResponse> => {
        const value = (request.params as Record<string, unknown>).value;
        return createMCPResponse(request.id, value);
      };
  ...
  describe('Timeout', () => {
    it('deve respeitar timeout configurado', async () => {
      const calls: ToolCall[] = [
        { server: 'test', method: 'test.slow', params: {} },
      ];
...
function createMockServer(name: string) {
  return {
    ...
    handleRequest: jest.fn().mockResolvedValue(
      createMCPResponse('test', { server: name })
    ),
