...
export class SleepAnalysisAgent extends BaseAgent {
  constructor() {
    super({
      ...
      capabilities: ['sleep-tracking', 'pattern-detection', 'deprivation-assessment', 'recommendations'],
      version: '1.0.0',
    });
  }
  ...
  private generateInsights(
    ...
    if (avgInterruptions > 5) {
      insights.push(`Você acorda em média ${Math.round(avgInterruptions)} vezes por noite. Isso é muito.`);
  ...
  private createAlerts(patterns: SleepPattern, currentState: any) {
    ...
    if (patterns.averageInterruptions > 6) {
      alerts.push({
        type: 'info',
        message: 'Muitas interrupções durante a noite. Converse com seu pediatra sobre o sono do bebê.',
      });
    }
