...
export class HabitsAnalysisAgent extends BaseAgent {
  constructor() {
    const config: AgentConfig = {
      ...
      version: '1.0.0',
      description:
        'Agente de análise de hábitos e bem-estar com insights baseados em IA',
      capabilities: [
        'habit-tracking',
        'pattern-detection',
        'trend-analysis',
  ...
  async process(
    ...
    try {
      ...
      await this.callMCP('analytics', 'event.track', {
        name: 'habits_analysis_requested',
        properties: {
          userId,
      ...
      const filteredEntries = timeRange
        ? this.filterByTimeRange(entries, timeRange)
        : entries;
      ...
      const overallScore =
        sleepQuality * 0.4 + moodScore * 0.3 + habitConsistency * 0.3;
      ...
      await this.callMCP('analytics', 'event.track', {
        name: 'habits_analysis_completed',
        properties: {
          userId,
   ...
   */
  private async analyzeHabitPatterns(
    entries: HabitEntry[]
  ...
  private detectTrend(
    ...
    const firstHalfRate =
      firstHalf.filter((e) => e.completed).length / firstHalf.length;
    const secondHalfRate =
      secondHalf.filter((e) => e.completed).length / secondHalf.length;
  ...
  private generateHabitInsights(
    ...
    if (completionRate >= 0.8) {
      ...
      insights.push(
        `"${habitName}" precisa de atenção. Que tal começar com metas menores?`
      );
    }
  ...
  private calculateSleepQuality(entries: HabitEntry[]): number {
    const sleepEntries = entries.filter((e) =>
      e.habitName.toLowerCase().includes('sono')
    );

    if (sleepEntries.length === 0) return 50; // Score neutro

    const completionRate =
      sleepEntries.filter((e) => e.completed).length / sleepEntries.length;
    const avgValue =
      sleepEntries
        .filter((e) => e.value !== undefined)
...
`;
  ...
  private detectAlerts(
    ...
    if (decliningHabits.length >= 2) {
      alerts.push(
        'Notamos que vários hábitos estão em declínio. Tudo bem com você?'
      );
    }
    ...
    const recentMoods = entries
      ...
      .filter((e) => e.mood)
      .map((e) => e.mood);
    const badMoodsCount = recentMoods.filter((m) =>
      ['bad', 'terrible'].includes(m!)
    ).length;
    ...
    const sleepPattern = patterns.find((p) =>
      p.habitName.toLowerCase().includes('sono')
    );
