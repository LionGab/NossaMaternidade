...
const AIModeItem: React.FC<AIModeItemProps> = React.memo(({
  ...
  const containerStyle = useMemo(() => ({
    backgroundColor: isSelected
      ? (isDark ? option.color + '30' : option.bgColor)
      : (isDark ? ColorTokens.overlay.glass : ColorTokens.overlay.card),
  ...
  return (
    ...
    >
      ...
      <RNText
        style={{
          ...styles.label,
          color: isSelected ? option.color : (isDark ? DarkTheme.text.secondary : LightTheme.text.secondary),
        }}
      >
        {option.label}
      </RNText>
