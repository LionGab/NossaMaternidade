...
export const EditableAvatar: React.FC<EditableAvatarProps> = ({
  ...
  const uploadAvatar = async (imageUri: string) => {
    try {
      ...
      if (error || !url) {
        const errorMsg = typeof error === 'string' ? error : error instanceof Error ? error.message : 'Erro ao fazer upload';
      ...
      const errorMessage = error instanceof Error ? error.message : 'Não foi possível atualizar a foto. Tente novamente.';
  ...
  const removeAvatar = async () => {
    try {
      ...
      if (!success) {
        const errorMsg = typeof error === 'string' ? error : error instanceof Error ? error.message : 'Erro ao remover foto';
        throw new Error(errorMsg);
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Não foi possível remover a foto. Tente novamente.';
      Alert.alert('Erro', errorMessage);
    } finally {
      setUploading(false);
    }
