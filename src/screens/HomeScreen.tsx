  ...
  return (
    <View
      ...
      style={[
        ...
        {
          backgroundColor: isDark ? 'rgba(11, 18, 32, 0.8)' : 'rgba(255, 255, 255, 0.8)',
...
const HabitCard: React.FC<HabitCardProps> = ({ habit, index, colors, onPress }) => {
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      accessibilityRole="button"
      accessibilityLabel={`Hábito: ${habit.custom_name || habit.habit?.name}. ${
        habit.today_completed ? 'Completado hoje' : `Sequência atual: ${habit.current_streak || 0} dias`
      }`}
...
const ContentCard: React.FC<ContentCardProps> = ({ item, colors, onPress }) => {
  ...
  return (
    ...
    >
      <View className="h-28 relative overflow-hidden" style={{ backgroundColor: colors.border.light }}>
        <Image
...
export default function HomeScreen() {
  ...
  const handleNavigate = useCallback(
    (screen: keyof RootStackParamList | keyof MainTabParamList) => {
      try {
        haptics.light();
        // Navegação pode ser para stack ou tab
        if (screen === 'Diary' || screen === 'Ritual') {
          // Telas do Stack - navegar diretamente
          (navigation as any).navigate(screen);
        } else {
          // Telas do Tab Navigator - navegar diretamente (já estamos no tab navigator)
          (navigation as any).navigate(screen);
        }
      } catch (error) {
