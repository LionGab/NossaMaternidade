...
const MetricCard: React.FC<{
  ...
  const statusColor = metric.color === 'success' ? colors.status.success :
                      metric.color === 'warning' ? colors.status.warning :
                      colors.status.error;
  ...
  return (
    ...
    >
      ...
      <Box direction="row" justify="space-between" align="center" mb="3">
        ...
        <TrendIcon 
          size={16} 
          color={metric.trend === 'up' ? colors.status.success : 
                 metric.trend === 'down' ? colors.status.error : 
  ...
  return (
    ...
    >
      ...
      <Box direction="row" gap="4">
        {/* Passed */}
        <Box flex={1} align="center" p="3" rounded="md" style={{ backgroundColor: `${colors.status.success}15` }}>
          <CheckCircle size={24} color={colors.status.success} />
          <Text size="xl" weight="bold" style={{ color: colors.status.success }}>
            {result.passed}
          </Text>
          <Text size="xs" color="tertiary">Passou</Text>
        </Box>

        {/* Warnings */}
        <Box flex={1} align="center" p="3" rounded="md" style={{ backgroundColor: `${colors.status.warning}15` }}>
          <AlertCircle size={24} color={colors.status.warning} />
          <Text size="xl" weight="bold" style={{ color: colors.status.warning }}>
            {result.warnings}
          </Text>
          <Text size="xs" color="tertiary">Avisos</Text>
        </Box>

        {/* Failed */}
        <Box flex={1} align="center" p="3" rounded="md" style={{ backgroundColor: `${colors.status.error}15` }}>
          <AlertCircle size={24} color={colors.status.error} />
          <Text size="xl" weight="bold" style={{ color: colors.status.error }}>
            {result.failed}
          </Text>
          <Text size="xs" color="tertiary">Falhou</Text>
        </Box>
      </Box>
      ...
      >
        ...
        >
          ...
          <Text size="sm" color="secondary">
            ...
            • <Text weight="semibold">Dark Mode:</Text> % de componentes com suporte a tema escuro{'\n'}
            • <Text weight="semibold">iOS/Android:</Text> Conformidade com guidelines de cada plataforma
          </Text>
        </Box>
      </ScrollView>
    </SafeAreaView>
