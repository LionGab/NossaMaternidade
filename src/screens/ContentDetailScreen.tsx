...
import { View, ActivityIndicator, ScrollView, ImageBackground, Dimensions } from 'react-native';
...
export default function ContentDetailScreen() {
  ...
  const loadContent = async () => {
    try {
      ...
      // TODO: Implementar feedService.getContentById(contentId) quando backend estiver pronto
      // Por enquanto, buscar da lista recomendada (workaround Week 1)
      const recommendedContent = await feedService.getRecommendedContent(20);
      const foundContent = recommendedContent?.find((item) => item.id === contentId);
  ...
  const handleLike = () => {
    setIsLiked(!isLiked);
    logger.info('Content liked toggled', { screen: 'ContentDetail', contentId, liked: !isLiked });
    // TODO: Integrar com backend para salvar like
  };

  const handleShare = () => {
    logger.info('Share content pressed', { screen: 'ContentDetail', contentId });
    // TODO: Implementar Share API
  };

  const handlePlay = () => {
    logger.info('Play content pressed', { screen: 'ContentDetail', contentId, type: content?.type });
  ...
  if (!content) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: colors.background.canvas, padding: Spacing['6'] }}>
    ...
    <View style={{ flex: 1, backgroundColor: colors.background.canvas }}>
      ...
      {content.thumbnail_url ? (
        <ImageBackground
          ...
          imageStyle={{ borderBottomLeftRadius: Radius['2xl'], borderBottomRightRadius: Radius['2xl'] }}
        >
          <Box
            style={{
              flex: 1,
          ...
          >
            ...
            >
              <Text size="sm" color="inverse" weight="semibold">
                {content.type === 'video' ? '🎥 Vídeo' : content.type === 'audio' ? '🎧 Áudio' : '📄 Artigo'}
              </Text>
            </Box>
          </Box>
        </ImageBackground>
      ...
      >
        ...
        {/* Metadados */}
        <Box style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing['3'], marginBottom: Spacing['4'], flexWrap: 'wrap' }}>
          {content.duration && (
