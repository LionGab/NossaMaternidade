...
export default function ProfileScreen({ navigation }: MainTabScreenProps<"Profile">) {
  ...
  const handleMenuItemPress = async (itemId: string) => {
    ...
    const menuConfig: Record<string, { title: string; description: string; emoji: string }> = {
      ...
      delete: {
        title: "Deletar Conta",
        description: "Esta ação é irreversível. Para solicitar a exclusão completa dos seus dados, entre em contato pelo email: privacidade@nossamaternidade.com.br. Conforme a LGPD, seus dados serão removidos em até 15 dias.",
        emoji: "⚠️",
      },
    };
          ...
          >
            {MENU_ITEMS.map((item, index) => (
              <Pressable
                ...
                accessibilityHint="Abre as configurações relacionadas"
              >
                <View
