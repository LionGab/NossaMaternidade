    ...
    lines.forEach((line, index) => {
      ...
      if (hexMatches) {
        hexMatches.forEach(match => {
          const suggestion =
            TOKEN_SUGGESTIONS[match.toUpperCase()] || TOKEN_SUGGESTIONS[match] || 'Use tokens do design system';
          violations.push({
            file: filePath,
            line: lineNumber,
            content: line.trim(),
...
export class DesignTokensValidationMCPServer implements MCPServer {
  ...
  private async validateTokens(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
  ...
  private async validateScreen(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
  ...
  private async suggestFix(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    if (violation.type === 'hex') {
      ...
      if (hexMatch) {
        ...
        if (suggestion) {
          ...
          if (violation.content.includes('backgroundColor:')) {
            after = violation.content.replace(hex, `colors.background.${suggestion.includes('background') ? 'canvas' : 'card'}`);
      ...
      if (spacingMatch) {
        ...
        const tokenKey = Object.keys({ 4: '1', 8: '2', 12: '3', 16: '4', 24: '6', 32: '8', 48: '12' }).find(
          k => parseInt(k, 10) === value
        );
        if (tokenKey) {
          const tokenValue = { 4: '1', 8: '2', 12: '3', 16: '4', 24: '6', 32: '8', 48: '12' }[tokenKey];
  ...
  private async checkDarkMode(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('#FFFFFF') || line.includes('#FFF') || line.includes("'white'") || line.includes('"white"')) {
        issues.push({
          file: fullPath,
          line: lineNumber,
          content: line.trim(),
      ...
      if (line.includes('#000000') || line.includes('#000') || line.includes("'black'") || line.includes('"black"')) {
        issues.push({
          file: fullPath,
          line: lineNumber,
          content: line.trim(),
