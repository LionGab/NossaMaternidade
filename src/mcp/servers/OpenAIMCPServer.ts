...
`;
  ...
  private async handleChat(
    ...
    switch (action) {
      case 'send':
      case 'crisis': { // Alias para tornar mais explícito o uso em crise
        const { message, context, history, model = 'gpt-4o' } = params as {
          message: string;
...
`;
        ...
        const completion = await this.openai.chat.completions.create({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: 'Você é um assistente especializado em triagem de risco emocional maternal.' },
