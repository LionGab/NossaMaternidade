...
`;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (!apiKey) {
        logger.warn('[AnthropicMCP] Anthropic API key not configured - server will not be available');
  ...
  private async handleChat(
    ...
    switch (action) {
      case 'send':
      case 'deep_think': { // Alias para raciocínio profundo
        const { message, context, history, model = 'claude-opus-4-20250514' } = params as {
          message: string;
...
`;
        ...
        const response = await this.anthropic.messages.create({
          ...
          max_tokens: 2048,
          temperature: 0.7,
          system: 'Você é um assistente especializado em análise de padrões comportamentais e emocionais maternais.',
