...
export class AccessibilityMCPServer implements MCPServer {
  ...
  private async handleA11y(id: string, action: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    switch (action) {
      case 'check.contrast':
  ...
  private async checkContrast(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
  ...
  private async checkTouchTargets(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    const fullPath = path.isAbsolute(componentPath) ? componentPath : path.join(process.cwd(), componentPath);
  ...
  private async checkLabels(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    const fullPath = path.isAbsolute(screenPath) ? screenPath : path.join(process.cwd(), screenPath);
    ...
    lines.forEach((line, index) => {
      ...
      if (
        ...
        for (let i = index; i < Math.min(index + 10, lines.length); i++) {
          ...
          if (lines[i].includes('</') && (lines[i].includes('TouchableOpacity') || lines[i].includes('Pressable'))) {
            break;
          }
        }
        ...
        if (!hasLabel) {
          missingLabels.push({
            file: fullPath,
            line: index + 1,
            component: line.match(/<(TouchableOpacity|Pressable|Button|TouchableHighlight)/)?.[1] || 'Interactive',
            suggestion: 'Adicione accessibilityLabel e accessibilityRole para melhorar acessibilidade',
          });
        }
      }
    });
  ...
  private async auditScreen(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const screenPath = params.screenPath as string;
    if (!screenPath) {
      return createMCPResponse(id, null, {
        code: 'MISSING_PARAM',
    ...
    const totalIssues =
      contrastIssues.length + touchTargetIssues.length + missingLabels.length + missingRoles + keyboardNavigation;
    const score = Math.max(0, 100 - totalIssues * 5); // Cada issue reduz 5 pontos

    const suggestions: string[] = [];
    if (touchTargetIssues.length > 0) {
      suggestions.push(`Corrija ${touchTargetIssues.length} problemas de touch target (mínimo 44pt)`);
    }
