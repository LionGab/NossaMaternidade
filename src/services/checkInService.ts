...
class CheckInService {
  ...
  async getCheckInHistory(limit = 30): Promise<CheckInLog[]> {
    try {
      ...
      logger.error('Unexpected error fetching check-in history', error, { service: 'checkInService' });
      return [];
    }
  }
  ...
  async getEmotionStats(): Promise<Record<EmotionValue, number>> {
    try {
      ...
      logger.error('Unexpected error calculating emotion stats', error, { service: 'checkInService' });
      return { bem: 0, triste: 0, ansiosa: 0, cansada: 0, calma: 0 };
    }
  }
}
