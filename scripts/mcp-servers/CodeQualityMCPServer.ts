...
export class CodeQualityMCPServer implements MCPServer {
  ...
  private async analyzeDesign(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    const totalIssues = hardcodedColors + hardcodedSpacing + hardcodedTypography + missingDarkMode + accessibilityIssues;
    ...
    if (hardcodedSpacing > 0) {
      suggestions.push(`Encontrado ${hardcodedSpacing} espaçamento hardcoded. Use Tokens.spacing.*`);
    }
    if (hardcodedTypography > 0) {
      suggestions.push(`Encontrada ${hardcodedTypography} tipografia hardcoded. Use Tokens.typography.*`);
    }
    if (accessibilityIssues > 0) {
      suggestions.push(`Encontrados ${accessibilityIssues} problemas de acessibilidade. Adicione accessibilityLabel.`);
    }
  ...
  private async findHardcoded(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
  ...
  private async suggestRefactor(
    ...
    const filePath = params.filePath as string;
    const violations = (params.violations as Array<{ type: string; line: number; content: string }>) || [];
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (violation.type === 'hex' || violation.type === 'rgba' || violation.type === 'rgb') {
          ...
          if (spacingMatch) {
            ...
            const tokenMap: Record<number, string> = { 4: '1', 8: '2', 12: '3', 16: '4', 24: '6', 32: '8', 48: '12' };
            const tokenValue = tokenMap[value];
            if (tokenValue) {
              after = line.replace(spacingMatch[0], `${spacingMatch[1]}: Tokens.spacing["${tokenValue}"]`);
          ...
          if (fontSizeMatch) {
            ...
            if (tokenValue) {
              after = line.replace(fontSizeMatch[0], `fontSize: Tokens.typography.sizes.${tokenValue}`);
