...
export interface SafetyIssue {
  ...
  line: number;
  content: string;
  issue: 'missing_disclaimer' | 'missing_crisis_detection' | 'missing_limitations' | 'unsafe_content';
...
export interface ClarityIssue {
  ...
  line: number;
  content: string;
  issue: 'missing_role' | 'missing_context' | 'missing_task' | 'missing_constraints' | 'unclear_structure';
  severity: 'critical' | 'warning' | 'info';
  suggestion: string;
}
    ...
    if (isSystemPrompt) {
      if (!hasDisclaimer) {
        issues.push({
          ...
          severity: 'critical',
          suggestion: 'Adicione disclaimer médico obrigatório (NÃO diagnostique, NÃO prescreva, NÃO substitua)',
        });
      }
      ...
      if (!hasCrisisDetection && content.length > 500) {
        ...
        issues.push({
          ...
          severity: 'warning',
          suggestion: 'Adicione detecção de crise emocional e recursos de emergência (CVV 188, SAMU 192)',
        });
      }
  ...
  try {
    ...
    if (content.length > 1000 && !content.includes('\n\n')) {
      issues.push({
        ...
        severity: 'info',
        suggestion: 'Considere dividir prompt em seções com quebras de linha para melhor legibilidade',
      });
    }
  } catch (error) {
    logger.error(`[PromptTestingMCP] Error reading file ${filePath}`, error);
...
function estimateTokenUsage(filePath: string, provider: 'gemini' | 'openai' | 'anthropic'): TokenEstimate | undefined {
  try {
    const content = fs.readFileSync(filePath, 'utf-8');
  ...
  try {
    ...
    testInputs.forEach(testInput => {
      ...
      if (hasCrisisKeywords) {
        ...
        const hasCrisisHandling = CRISIS_DETECTION_PATTERNS.some(pattern => pattern.test(content));
...
export class PromptTestingMCPServer implements MCPServer {
  ...
  private async validateSafety(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    ...
    const fullPath = path.isAbsolute(promptPath) ? promptPath : path.join(process.cwd(), promptPath);
  ...
  private async validateClarity(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const promptPath = params.promptPath as string | undefined;
  ...
  private async testTokens(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const promptPath = params.promptPath as string | undefined;
  ...
  private async testCrisis(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const promptPath = params.promptPath as string | undefined;
  ...
  private async validateFallback(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const promptPath = params.promptPath as string | undefined;
  ...
  private async validateAll(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const promptPath = params.promptPath as string | undefined;
    const provider = (params.provider as 'gemini' | 'openai' | 'anthropic') || 'gemini';
