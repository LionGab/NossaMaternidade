...
export interface FlatListIssue {
  ...
  line: number;
  content: string;
  issue: 'missing_keyExtractor' | 'missing_getItemLayout' | 'missing_windowSize' | 'missing_removeClippedSubviews';
  ...
  try {
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        ...
        if (
          !['node_modules', '.expo', 'dist', 'build', 'android', 'ios', 'web-build', '__tests__'].includes(dirName)
        ) {
        ...
        if (line.includes('/>') || (line.includes('</FlatList') || line.includes('</FlashList'))) {
          ...
          if (!hasGetItemLayout && content.includes('renderItem')) {
            issues.push({
              ...
              severity: 'warning',
              suggestion: 'Adicione getItemLayout para itens de altura fixa (melhora scroll performance)',
            });
          }
          ...
          if (!hasRemoveClippedSubviews) {
            issues.push({
              ...
              severity: 'info',
              suggestion: 'Considere adicionar removeClippedSubviews={true} para melhorar performance',
            });
          }
    ...
    lines.forEach((line, index) => {
      ...
      if (
        ...
        if (componentMatch) {
          ...
          if (content.includes(componentName) && (content.includes('renderItem') || content.includes('map('))) {
            issues.push({
              file: filePath,
              line: lineNumber,
...
function checkImageOptimization(filePath: string): ImageIssue[] {
  ...
  try {
    ...
    lines.forEach((line, index) => {
      ...
      if (IMAGE_PATTERN.test(line) && !FAST_IMAGE_PATTERN.test(content) && !line.includes('FastImage')) {
        issues.push({
          file: filePath,
          line: lineNumber,
          content: line.trim(),
      ...
      if (IMAGE_PATTERN.test(line) && !IMAGE_RESIZE_PATTERN.test(line) && !line.includes('source')) {
      ...
      if (line.includes('http') && (line.includes('.jpg') || line.includes('.png') || line.includes('.jpeg'))) {
        if (line.length > 200) {
          issues.push({
            file: filePath,
            line: lineNumber,
      ...
      if (inLoop && SUPABASE_QUERY_PATTERN.test(line)) {
        ...
        if (queryCount > 1) {
          issues.push({
            ...
            issue: 'n_plus_one',
            severity: 'critical',
            suggestion: 'Múltiplas queries dentro de loop detectadas (N+1 problem). Use batch queries ou joins',
          });
        }
      }
...
export class MobileOptimizationMCPServer implements MCPServer {
  ...
  private async checkFlatList(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
  ...
  private async checkMemo(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const filePath = params.filePath as string | undefined;
  ...
  private async checkImages(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const filePath = params.filePath as string | undefined;
  ...
  private async analyzeBundle(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const filePath = params.filePath as string | undefined;
  ...
  private async checkQueries(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const filePath = params.filePath as string | undefined;
  ...
  private async analyzeAll(id: string, params: Record<string, unknown>): Promise<MCPResponse<JsonValue>> {
    const filePath = params.filePath as string | undefined;
