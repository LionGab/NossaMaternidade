...
const TOKENS_IMPORT = "import { Tokens, useThemeColors } from '@/theme';\nimport { Spacing, Radius, Typography } from '@/theme/tokens';";
...
function detectColorViolations(content) {
  ...
  lines.forEach((line, index) => {
    ...
    const namedMatch = line.match(/\b(color|backgroundColor|borderColor|shadowColor)\s*:\s*['"](white|black)['"]/);
    if (namedMatch) {
...
function detectSpacingViolations(content) {
  ...
  const spacingPattern = /\b(padding|margin|gap|top|bottom|left|right|width|height|minWidth|minHeight|maxWidth|maxHeight)\s*:\s*(\d+)/g;
  const lines = content.split('\n');
...
function applyFixes(content, violations, options) {
  ...
  if (appliedFixes.length > 0 && !fixedContent.includes("from '@/theme'") && !fixedContent.includes("from '@/theme/tokens'")) {
    ...
    const importsToAdd = "import { useThemeColors } from '@/theme';\nimport { Spacing, Radius, Typography } from '@/theme/tokens';";
