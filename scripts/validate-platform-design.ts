...
function validateIOSFonts(filePath: string, content: string): void {
  ...
  if (filePath.includes('tokens.ts') || 
...
function validateIOSSafeAreas(filePath: string, content: string): void {
  ...
  if (content.includes('SafeAreaView') || content.includes('SafeAreaContainer') || content.includes('useSafeAreaInsets')) {
    hasSafeArea = true;
  }
...
function validateIOSHaptics(filePath: string, content: string): void {
  ...
  if (filePath.includes('primitives') || filePath.includes('Button') || filePath.includes('Pressable')) {
    if (content.includes('onPress') && 
      ...
      violations.push({
        ...
        line: 1,
        type: 'warning',
        message: 'iOS: Componentes interativos devem usar triggerPlatformHaptic() para feedback tátil',
        platform: 'ios',
      });
    }
  }
...
function validateAndroidFonts(filePath: string, content: string): void {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.includes('fontFamily') && !line.includes('Roboto') && !line.includes('System') && !line.includes('PlatformFonts')) {
      violations.push({
        file: filePath,
        line: lineNumber,
        type: 'warning',
...
function validateAndroidElevation(filePath: string, content: string): void {
  ...
  content.split('\n').forEach((line, index) => {
    ...
    if ((line.includes('shadowColor') || line.includes('shadowOffset')) && 
        !line.includes('getPlatformShadow') && 
        !line.includes('getIOSShadow') &&
        !line.includes('getAndroidElevation') &&
        !line.includes('// TODO') &&
        (line.includes(':') || line.includes('='))) { // É uma atribuição/definição
      violations.push({
        file: filePath,
        line: lineNumber,
        type: 'warning',
        message: 'Android: Use getPlatformShadow() ou getAndroidElevation() em vez de shadowColor/shadowOffset',
        platform: 'android',
      });
    }
  });
...
function validateAndroidRipple(filePath: string, content: string): void {
  ...
  if (content.includes('<Pressable') && 
    ...
    violations.push({
      ...
      line: 1,
      type: 'warning',
      message: 'Android: Pressable deve ter android_ripple configurado para feedback Material Design',
      platform: 'android',
    });
  }
}
  ...
  if (content.includes('from \'react-native\'') && 
    ...
    if (content.includes('<Text') || content.includes('Text>')) {
      violations.push({
        ...
        line: 1,
        type: 'warning',
        message: 'Text components devem ter allowFontScaling para suportar Dynamic Type (iOS) e Text Scaling (Android)',
        platform: 'both',
      });
    }
  }
  ...
  function getAllFiles(dir: string, fileList: string[] = []): string[] {
    ...
    files.forEach(file => {
      ...
      try {
        ...
        if (stat.isDirectory() && !['node_modules', '__tests__', '.expo', 'dist', 'build'].includes(file)) {
