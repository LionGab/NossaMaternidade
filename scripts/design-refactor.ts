...
class DesignRefactorEngine {
  ...
  analyzeFile(filePath: string): DesignViolation[] {
    ...
    lines.forEach((line, index) => {
      ...
      const namedColorRegex = /\b(white|black|red|blue|green|yellow|orange|pink|purple|gray|grey)\b/gi;
      ...
      const spacingRegex = /(?:padding|margin|gap|top|bottom|left|right|width|height):\s*(\d+)(?!px|pt|dp|rem|em)/g;
  ...
  private suggestTokenForFontSize(value: number): string {
    ...
    const keys = Object.keys(fontSizeMap).map(Number).sort((a, b) => a - b);
    const closest = keys.reduce((prev, curr) => 
      Math.abs(curr - value) < Math.abs(prev - value) ? curr : prev
    );
